...
- `segments[].replaceItems` で、`multi_select` の特定選択肢を別fieldの入力値へ置換できる。たとえば「その他」を自由記載の薬剤名へ置き換え、1行にまとめて出力する
...
- Simple Editorの項目カードは、閉じた状態でも右端の削除ボタンから項目を削除できる
...
脳卒中共通テンプレート（`neuro_common`）は `generic-v2` として条件分岐に対応済みです。`multi_select` と `number` を使い、意識レベル、バイタル、瞳孔・眼球所見、運動機能、NIHSS、高次脳機能、頭蓋内圧亢進症状、嚥下、安静度・ADL、排泄、治療を観察項目として扱います。慢性硬膜下血腫テンプレート（`chronic_subdural`）も `generic-v2` として追加済みです。派生出力は `multi-v1` として実装済みです。状況グループは次フェーズ以降で扱います。
...
- `templates`
  ...
  - `current_version_id` は同じtemplateの `published` versionだけを指せる

- `template_versions`
  - テンプレートの入力項目JSONと変更理由をバージョンとして保存
  - `status` は `draft` / `published` / `retired`
  - 既存行を直接上書きせず、新しいバージョンを作る
...
- `POST /api/templates`
  ...
  - `id` は `^[a-z0-9_-]{1,32}$`
- `POST /api/templates/<id>/versions`
  - 既存テンプレートを上書きせず、新しい下書きバージョンとして編集内容を追加する
  ...
  - 作成直後は `draft` で、通常画面には反映されない
- `POST /api/templates/<id>/versions/<version_id>/publish`
  - 下書きversionを公開し、`templates.current_version_id` を切り替える
  - 高リスク変更がある場合は `confirm_high_risk: true` がないと `409` を返す
- `POST /api/templates/<id>/versions/<version_id>/rollback`
  - 指定versionの内容を複製した新versionを作成し、その新versionを公開する
  - 過去versionへ単純にpointerを戻さず、公開履歴の時系列を保つ
- `POST /api/templates/<id>/delete`
...
編集保存時は下書きversionを作成します。履歴画面から下書きを公開すると通常画面へ反映されます。過去版の復元は、過去versionへ直接戻すのではなく、過去versionの内容を複製した新しいpublished versionを作成します。
...
- 本番用WSGIサーバーで起動する
- DBバックアップと復旧手順を決める
- 外部公開前に、権限・監査ログ閲覧権限・承認方針を決める

## 次のコミット候補

- 状況グループ設計
- 承認フロー設計
- 旧 `templates.schema_json` 互換の整理
