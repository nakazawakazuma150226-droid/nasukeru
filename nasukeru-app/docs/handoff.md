...
- `014`: 脳卒中共通の降圧薬「その他」を自由記載の薬剤名へ置き換え、単一行でコピー出力
...
copy lineの `segments[].replaceItems` は、`multi_select` の選択値を別fieldの入力値へ置換します。脳卒中共通では「その他」を選択した場合、`降圧薬：その他` と別行を出さず、入力した薬剤名を `降圧薬：薬剤名` の1行として出力します。
